...
Bulk review: select individual drafts on the visible page (up to 50). The action bar appears only while at least one draft is selected and shows the selected count, Approve selected, and Clear selection. Full draft bodies appear on cards and again on the confirmation page. Selections reset on navigation/filter changes. Confirmation tokens bind each draft to its reviewed delivery envelope; every approval still uses the existing transaction/version check and audit trail. Results report successes, changed/stale drafts, and unconfirmed failures individually. No Gmail API is called by bulk approval. Enveloped approvals queue send intents; drafts without prepared delivery details receive review-only approval.
